...
class NFLTDBoostCalculator:
    ...
    def calculate_league_averages(self):
        """Calculate 2024 league-wide averages for boost comparisons (match local)."""
        ...
        for (game_id, team), g in rz.groupby(['game_id', 'posteam'], dropna=False):
            ...
            for d_id, d in g.groupby('fixed_drive'):
                if len(d) >= 2:
                    all_rz_rows.append({'side': 'off', 'td': int(d['touchdown'].max())})
        # Defense side
        for (game_id, team), g in rz.groupby(['game_id', 'defteam'], dropna=False):
            ...
            for d_id, d in g.groupby('fixed_drive'):
                if len(d) >= 2:
                    all_rz_rows.append({'side': 'def', 'td': int(d['touchdown'].max())})
        ...
        if rz_df.empty:
            ...
            off_rz = rz_df[rz_df['side'] == 'off']
            def_rz = rz_df[rz_df['side'] == 'def']
    ...
    def load_data(self):
        """Load both 2024 baselines and 2025 current data (match local)."""
        try:
            # 1) League averages from real 2024 data
            self.calculate_league_averages()

            # 2) 2024 baselines (off/def, RZ & all drives)
            ...
            # 3) 2025 current (off/def, RZ & all drives)
        ...
        try:
            ...
            if self.schedule_data is not None:
                try:
                    ...
                    est = timezone(timedelta(hours=-5))  # EST offset (no DST handling)
    ...
    def calculate_matchup_boosts(self, offense_team, defense_team):
        """Calculate TD boost for a specific matchup with percentage changes and detailed labels (match local)."""
        # Ensure BOTH baselines and 2025 current exist
